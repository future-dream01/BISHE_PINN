...
class Train_Dataset(Dataset):
    def __init__(self, csv_path):
        ...
        # ===================== 核心：Omega 对数归一化 =====================
        OMEGA_OUTPUT_IDX = 6   # 输出7列中，Omega在索引6
        OMEGA_TOTAL_IDX = 10   # 总11列中，Omega在索引10
        
        # 1. 提取原始无量纲Omega
        omega_raw = self.outputs[:, OMEGA_OUTPUT_IDX]
        # 2. 对数变换（压缩量级）
        ln_omega = np.log(omega_raw)
        # 3. 计算对数后的极值
        ln_omega_min = np.min(ln_omega).astype(np.float32)
        ln_omega_max = np.max(ln_omega).astype(np.float32)
        # 4. ✅ 直接覆盖原input_min/max中Omega的位置（总列索引10）
        self.input_min[OMEGA_TOTAL_IDX] = ln_omega_min
        self.input_max[OMEGA_TOTAL_IDX] = ln_omega_max
        # 5. 0-1归一化
        omega_norm = (ln_omega - ln_omega_min) / (ln_omega_max - ln_omega_min + 1e-8)
        # 6. 替换输出中的Omega
        self.outputs[:, OMEGA_OUTPUT_IDX] = omega_norm
        ...
        print(f"   原始Omega范围: {omega_raw.min():.4f} ~ {omega_raw.max():.4f}")
        print(f"   对数后Omega范围: {ln_omega.min():.4f} ~ {ln_omega.max():.4f}")
        print(f"   已覆盖input_min[10]={self.input_min[10]:.4f}, input_max[10]={self.input_max[10]:.4f}")
...
class Val_Dataset(Dataset):
    def __init__(self, csv_path, input_min=None, input_max=None):
        ...
        ln_omega = np.log(omega_raw)
        # 3. ✅ 严格使用训练集的对数极值做归一化
        ln_omega_min = self.input_min[OMEGA_TOTAL_IDX]
        ln_omega_max = self.input_max[OMEGA_TOTAL_IDX]
        omega_norm = (ln_omega - ln_omega_min) / (ln_omega_max - ln_omega_min + 1e-8)
        # 4. 替换输出中的Omega
        self.outputs[:, OMEGA_OUTPUT_IDX] = omega_norm
        ...
        print(f"\n✅ 验证集加载完成")
        print(f"   使用训练集对数Omega极值：ln_min={ln_omega_min:.4f}, ln_max={ln_omega_max:.4f}")
        print(f"   实际采样数据点数: {len(self.inputs)}")
