...
class Train_Dataset(Dataset):
    def __init__(self, csv_path):
        ...
        # 【修改】输入：从4列扩展为6列 (X,Y,Z,d,Ma,Pr)
        self.inputs = df.iloc[:, :6].values.astype(np.float32)   
        # 【修改】输出：从第4列开始改为从第6列开始 (U,V,W,P,T,K,Omega)
...
def data_prepare(batchsize, train_csv_path=None, val_csv_path=None):
    if train_csv_path is None:
        train_csv_path = os.path.join(project_root, "datasets/csv/train.csv") # 可修改为你的默认路径
    if val_csv_path is None:
        val_csv_path = os.path.join(project_root, "datasets/csv/val.csv")   # 可修改为你的默认路径
